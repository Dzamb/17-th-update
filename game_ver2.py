...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        self.wall_list = arcade.tilemap.process_layer(my_map, platform_layer_name, TILE_SCALING)
        self.background_list = arcade.tilemap.process_layer(my_map, background_layer_name, TILE_SCALING)
        self.vilage_list = arcade.tilemap.process_layer(my_map, vilage_layer_name, TILE_SCALING)
        self.coin_list = arcade.tilemap.process_layer(my_map, coin_layer_name, TILE_SCALING)
        ...
        c_list = arcade.tilemap.process_layer(my_map, coin_layer_name, TILE_SCALING)
        for c in c_list:
            coin_sprite = arcade.AnimatedTimeSprite(1)
            coin_sprite.textures = []
            coin_sprite.textures.append(arcade.load_texture())
        ...
        for e in e_list:
            enemy_sprite = Goblin()
        enemy_sprite.texture_change_distance = 20

        enemy_sprite.center_x = e.center_x
        enemy_sprite.center_y = e.center_y + 64
        enemy_sprite.scale = ENEMY_SCALE
        enemy_sprite.change_x = -ENEMY_SPEED
